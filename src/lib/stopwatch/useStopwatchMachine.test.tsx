...
describe('useStopwatchMachine', () => {
  ...
  describe('rehydrating from localStorage (simulates a page reload)', () => {
    ...
    it('does NOT re-acquire the wake lock at launch when rehydrated running (07e native, #131 web)', () => {
      // A rehydrated running stopwatch must not pin the screen at app launch — the mount fires on
      // whatever route the app loads on (Home/Settings/presets), and re-grabbing the lock there is
      // exactly the #131 bug. The rehydrated-running hold is tied to the Timer screen being shown
      // instead (Timer.tsx's 'stopwatch-screen' owner). Platform-independent: no launch re-acquire
      // on web or native.
      const acquireSpy = vi.spyOn(wakeLock, 'acquire').mockResolvedValue();
    ...
    it('the launch skip does not disable the stopwatch owner — a later resume still re-acquires it', () => {
      // Guards against a future refactor over-broadening the launch-skip into disabling the
