...
> **On the web this is self-limiting; on Android (Phase 7) it is not.** The behaviour above is benign in a browser because `navigator.wakeLock` is only granted to a _visible_ document and the platform auto-releases it on hide — so a stale rehydrated `running` stopwatch can't actually keep a backgrounded tab's screen awake. The Phase 7 Android build backs `wakeLock.ts` with a native keep-awake plugin whose flag is **not** self-limiting: the same rehydrate-and-reacquire-on-any-route path would hold the screen on across unrelated screens until the user returns and resets. Phase 7 therefore has to make the native re-acquire policy a _deliberate_ choice (only hold while the running stopwatch is on screen, or expire a long-stale `running` state) rather than inherit these web semantics. See [SPECIFICATIONS/07-android-app.md](../../SPECIFICATIONS/07-android-app.md) → the keep-awake decision. Noted here so anyone touching this rehydrate path knows the web's free safety net doesn't exist on native.

> **Update (#131, 2026-08-10):** the "self-limiting on web" framing above was weaker than it read — on a cold load that lands on Home, the document _is_ visible, so the launch re-acquire pinned the screen even on web. The **launch re-acquire in `useStopwatchMachine` described in the two paragraphs above has been removed** (on both platforms); the rehydrated-running hold is now tied to the Timer screen being shown, via `Timer.tsx`'s `stopwatch-screen` owner. So the "re-acquires it explicitly on mount" mechanism no longer exists — treat those paragraphs as the original rationale, not current behaviour. Full detail in the [wake-lock ADR](./2026-08-08-native-wakelock-keepawake.md) "#131 landed" amendment.
