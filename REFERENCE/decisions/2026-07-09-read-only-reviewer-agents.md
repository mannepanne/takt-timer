...
- **Restrict reviewer agents' `tools:` frontmatter to remove `Bash`.**
  - Why not: reviewers genuinely need `gh pr diff`, `gh pr view`, and `git show`. The frontmatter gates tool _names_, not individual shell commands, so this is all-or-nothing and would break context gathering entirely.
