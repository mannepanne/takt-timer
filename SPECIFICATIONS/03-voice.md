...
- **PR #17 (B3c safety harness)** — three independent safety nets that codify Phase 3 promises before Phase 4 adds the first deliberate user-data writes. No-persistence regression test in `worker/api/voice/parse.test.ts` probes `DB`/`AUDIO_BUCKET`/`USER_DATA` bindings on both the success path and the schema-failed error branch, asserting the only KV write is the rate-limit counter. Adversarial-transcript test stubs Llama to return `sets:9999` and asserts the zod `SessionSchema` rejects with `schema-failed` (never a `parsed` event). Cold-start `AbortController` in `src/lib/voice/voice-client.ts` opens a 30s window scoped to first-body-line arrival (not header arrival — the Worker uses a TransformStream so headers come back in milliseconds while the body waits on Workers AI); a fired timeout dispatches `errorArrived(reason: 'cold-start-timeout')`, which routes via `parse-error` to the dedicated overlay copy. `parseErrorCopy` switched to an exhaustive `switch` so future `ErrorReason` additions force a conscious copy decision.

**Pending verification:** real-device smoke test of the B2 + B3b + B3c flow on iPhone + Android using the manual checklist below — including the B3c cold-start path (cold Worker instance → first call after several minutes of idleness should still complete within 30s; if not, the cold-start-timeout copy appears). Deploy completed 2026-04-20 via `.github/workflows/deploy.yml`. If smoke tests reveal regressions, file fixes against the next branch.

**Remaining work — exit criteria (next session entry point):**

- Remaining architecture-hygiene items in the Pre-commit checklist below (status-code client contract doc, `requestMic` effect refactor).
