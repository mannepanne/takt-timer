...
> _Superseded by #153 (2026-09-13): the circular ring is gone. Tester feedback (issue #153) was that the running screens needed to read from across the room, so the Timer screen now shows 120px digits with a thin per-minute progress bar at the top (accent fill, `minuteProgress` replacing `ringProgress`), matching the run screens' bar. `ProgressRing.tsx` and `ringProgress` were deleted. The "barely moves on a single set" caveat below is exactly why the hour ring was dropped. Left in place as the original archived rationale; see [at-a-distance-readability.md](../at-a-distance-readability.md)._
