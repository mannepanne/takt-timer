...
- **Unnumbered feature specs** sit at the root of this folder while in progress — small post-launch features that don't belong to the original phase sequence. Same "When a phase ships" archiving rule applies once merged and verified in production, e.g. [timer-mode.md](./ARCHIVE/timer-mode.md) and [dark-mode.md](./ARCHIVE/dark-mode.md) (System / Light / Dark appearance for web and Android, issue #154, three PRs #157 → #158 → #160), both shipped and archived. Active now: [at-a-distance-readability.md](./at-a-distance-readability.md) — bigger phase/set cues, a tinted rest background and a ring-free Timer page so the running screens read from across the room (issue #153).
