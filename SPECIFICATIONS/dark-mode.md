...
- _Two latent-bug fixes._ `.voice-overlay-sheet` already declared `background: var(--surface)` against a token that didn't exist, so it rendered _transparent_ over the blurred scrim; defining `--surface` gives it the white card the prototype intended. `--ink-muted` (`.voice-overlay-body`, `.accent-picker-label`) was undefined, so that text inherited full `--ink`; it now resolves to `--ink-3`.
