...
**Two latent-bug fixes that change light mode slightly — both deliberate, both called out in the PR body.** `.voice-overlay-sheet` already declared `background: var(--surface)` against a token that didn't exist, so it rendered _transparent_ over the blurred scrim; defining `--surface` gives it the white card the prototype intended. And `--ink-muted` (`.voice-overlay-body`, `.accent-picker-label`) was undefined, so that text inherited full `--ink`; it now resolves to `--ink-3`, the muted shade the name promises.
...
- **Contrast is a no-regression check, not an absolute bar.** `scripts/check-contrast.mjs` computes WCAG ratios for a **hand-listed** set of text-token × surface-token pairs actually used in the app (the list is in the script and drifts if nobody maintains it — which is why this is a PR artefact and must not become a CI gate at this specificity), plus each accent's `--accent-deep` on `--paper` and `--surface`, in both appearances. Each pair is judged at the WCAG threshold for how it is used — **4.5:1 for text (1.4.3), 3:1 for an icon or control on a filled shape (1.4.11)**; the only white-on-accent and white-on-danger uses are icon buttons, so those are 3:1 pairs and pass in both palettes. The criterion: **every pair that passes in light also passes in dark.** A drop that stays above threshold (17:1 → 15:1) is reported, not failed — otherwise every dark palette "regresses" against an off-white page. The light palette already fails 4.5:1 on `--mute` (2.85:1), `--warn`, `--success`, `--danger` on `--paper-2`, and accent-coloured link text (`.settings-link`, `.privacy-link`) for all six accents; those are seeded, commented exemptions — pre-existing debt, recorded, not fixed here. (Every one of them except `--mute` on `--surface` passes in dark.) Output pasted into the PR description.
...
- [ ] Light mode is unchanged except the three documented deltas — the `--hover-tint` merge, the voice-overlay sheet gaining its intended white card, and `--ink-muted` resolving to `--ink-3` — confirmed by the CSS diff being pure token substitution plus screenshots of Home, Run and Settings.
- [ ] `scripts/check-contrast.mjs`: every pair that passes in light passes in dark at its use-appropriate threshold; exemptions listed; output in the PR.
