...
| [07g](./07g-back-button-copy.md)   | **Back button, lifecycle, copy forks** — `backButton`/`appStateChange` wiring, `Privacy`/`Onboarding`/`Settings`                           | 07a·2, 07e                            | Yes                         | firms up after 07a·2         |
...
Respect the gating in the [Delivery plan](#delivery-plan) table: spikes (07a) first, then the `07b→07c→07d` spine, with `07e`/`07f` in parallel once their spikes clear, `07g` after `07e`, and `07h`'s admin track running on its own calendar clock from day one.
