...
7. [07-android-app.md](./SPECIFICATIONS/07-android-app.md) — 🚧 in progress — Android app via Capacitor, sold on Google Play, device-scoped presets, no accounts. Umbrella spec; broken into eight child deliverables `07a`–`07h` (spikes → scaffold → auth no-op → presets → wake-lock → voice → back-button/copy → publishing), shipped as a sequence of PRs. The `07a` spikes (keep-awake / lifecycle / speech) all passed on real hardware, and the `07b` Capacitor scaffold has landed — the native `android/` project builds an APK that installs and runs on a device. **All Phase 7 _code_ (`07a`–`07h`) has landed and is device-verified on real hardware** — `07a` spikes, `07b` scaffold, `07c` (auth/network no-op), `07d` (device-local presets), `07e` (native wake-lock via keep-awake), `07f` (native on-device voice — system recogniser + local English parser + fail-safe manual fallback), `07g` (copy forks + hardware back-button/app-lifecycle wiring), and `07h`'s code/release track (release signing pipeline for a signed AAB, per-platform privacy-policy URLs, the `store-assets/` listing library). **Remaining: only `07h`'s Play Console _admin track_** — developer/merchant account, ≥12 testers, the 14-day closed test — which is calendar time, not code. **The closed test is now underway:** ≥12 testers are opted in and the 14-day clock started on 12 Sep 2026, so the earliest eligibility for production access is 26 Sep 2026 (assuming ≥12 testers stay continuously opted in).

**Post-launch feature work:** once all six phases are live, further features get their own spec in [SPECIFICATIONS/](./SPECIFICATIONS/), outside the numbered sequence, while in progress. Once shipped and verified, it's archived like any other spec — e.g. [timer-mode.md](./SPECIFICATIONS/ARCHIVE/timer-mode.md) (count-up stopwatch for rep-based exercises; see [ADR 2026-08-02](./REFERENCE/decisions/2026-08-02-timer-mode-provider-scoped-state.md)). Likewise [dark-mode.md](./SPECIFICATIONS/ARCHIVE/dark-mode.md) — System / Light / Dark appearance for web and Android (issue #154), shipped as three PRs (#157 tokens, #158 setting, #160 native chrome); see [ADR 2026-09-12](./REFERENCE/decisions/2026-09-12-theme-resolution.md) and [REFERENCE/theming.md](./REFERENCE/theming.md).

**Current phase:** The original six web-app phases are complete and live, and small post-launch features ship as unnumbered specs (see above). Phase 7 — an Android app via Capacitor — has **all code complete and device-verified** (`07a`–`07g`): the native app installs, runs offline, keeps the screen awake while a timer runs, saves/browses device-local presets, does voice-driven setup via the on-device recogniser with a fail-safe manual fallback, and handles the hardware back button + background-pause natively. `07h`'s code/release track has also landed (signed-AAB signing pipeline, per-platform privacy URLs, `store-assets/` library). **Only `07h`'s Play Console admin track remains** — developer/merchant account, ≥12 testers, the 14-day closed test (calendar time, not code). **The closed test is running:** ≥12 testers opted in, 14-day clock started 12 Sep 2026, earliest production-access eligibility 26 Sep 2026 (provided ≥12 stay continuously opted in).
