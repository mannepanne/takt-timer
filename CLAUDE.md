...
7. [07-android-app.md](./SPECIFICATIONS/07-android-app.md) — 🚧 in progress — Android app via Capacitor, sold on Google Play, device-scoped presets, no accounts. Umbrella spec; broken into eight child deliverables `07a`–`07h` (spikes → scaffold → auth no-op → presets → wake-lock → voice → back-button/copy → publishing), shipped as a sequence of PRs. The `07a` spikes (keep-awake / lifecycle / speech) all passed on real hardware, and the `07b` Capacitor scaffold has landed — the native `android/` project builds an APK that installs and runs on a device. Landed: `07a` spikes, `07b` scaffold, `07c` (auth/network no-op), `07d` (device-local presets — save from Complete + browse in the drawer, fully offline), `07g`'s copy forks (account/passkey copy removed from native Settings, Onboarding, Privacy), `07e` (native wake-lock via keep-awake, device-verified), and `07f` (native on-device voice — system recogniser + local English parser + fail-safe manual fallback, device-verified). Remaining: `07g`'s hardware back-button + lifecycle (gated on `07e`), and `07h` publishing.
...
**Current phase:** The original six web-app phases are complete and live, and small post-launch features ship as unnumbered specs (see above). Phase 7 — an Android app via Capacitor — is **in progress**: `07a`–`07g`'s copy forks plus `07e` (wake-lock) and `07f` (native voice) have landed and are device-verified on real hardware — the native app installs, runs offline, keeps the screen awake while a timer runs, saves/browses device-local presets, and does voice-driven setup via the on-device recogniser with a fail-safe manual fallback. Remaining: `07g`'s hardware back-button + lifecycle (gated on `07e`, now unblocked), and `07h` publishing.
