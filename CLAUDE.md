...
**Current phase:** The original six web-app phases are complete and live, and small post-launch features ship as unnumbered specs (see above). Phase 7 — an Android app via Capacitor — is **in progress**: the device-independent module cores (`07f` voice parser, `07d` local presets store) are merged; the remaining work (Capacitor scaffold `07b`, the `07a` spikes, native wiring, and publishing) is gated on the Android toolchain and a physical test device.
