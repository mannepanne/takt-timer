...
7. [07-android-app.md](./SPECIFICATIONS/07-android-app.md) — 🚧 in progress — Android app via Capacitor, sold on Google Play, device-scoped presets, no accounts. Umbrella spec; broken into eight child deliverables `07a`–`07h` (spikes → scaffold → auth no-op → presets → wake-lock → voice → back-button/copy → publishing), shipped as a sequence of PRs. The `07a` spikes (keep-awake / lifecycle / speech) all passed on real hardware, and the `07b` Capacitor scaffold has landed — the native `android/` project builds an APK that installs and runs on a device. Landed: `07a` spikes, `07b` scaffold, `07c` (auth/network no-op), `07d` (device-local presets — save from Complete + browse in the drawer, fully offline), and `07g`'s copy forks (account/passkey copy removed from native Settings, Onboarding, Privacy). Remaining: `07e` wake-lock, `07f` native voice wiring, `07g`'s hardware back-button + lifecycle (gated on `07e`), and `07h` publishing.
...
**Current phase:** The original six web-app phases are complete and live, and small post-launch features ship as unnumbered specs (see above). Phase 7 — an Android app via Capacitor — is **in progress**: the `07a` spikes passed on real hardware, the `07b` Capacitor scaffold has landed (the `android/` project builds an APK that installs and runs on a device), `07a`–`07d` have landed (scaffold, auth no-op, device-local presets with native save + browse) along with `07g`'s copy forks. Remaining: `07e` wake-lock, `07f` native voice wiring, `07g`'s back-button + lifecycle (gated on `07e`), and `07h` publishing.
