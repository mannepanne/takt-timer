...
7. [07-android-app.md](./SPECIFICATIONS/07-android-app.md) — 🚧 in progress — Android app via Capacitor, sold on Google Play, device-scoped presets, no accounts. Umbrella spec; broken into eight child deliverables `07a`–`07h` (spikes → scaffold → auth no-op → presets → wake-lock → voice → back-button/copy → publishing), shipped as a sequence of PRs. The `07a` spikes (keep-awake / lifecycle / speech) all passed on real hardware, and the `07b` Capacitor scaffold has landed — the native `android/` project builds an APK that installs and runs on a device. Module cores `07f` (voice parser) and `07d` (presets store) are merged. Remaining: native wiring (`07c` auth no-op, `07e` wake-lock, `07f`/`07g` voice + back-button + copy forks) and `07h` publishing.
