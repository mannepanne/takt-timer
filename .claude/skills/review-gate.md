...
- **`"disabled"`** → reply with this line, substituting the invoking skill's name: _"The review system is disabled for this project (set via `prReviewMode` in `.claude/project-config.json`). Not running `/<skill-name>`. To re-enable, change the flag to `\"enabled\"`."_ Stop. Do not continue into the skill.
...
- **Write target.** If `.claude/project-config.local.json` exists, write to _that_ file (the presence of a local override file is a signal the user wants their changes kept local). Otherwise write to the committed `.claude/project-config.json`.
...
Lead-in line (always render _before_ the blockquote, on its own line, plain text — not part of the quote):

> The project's `prReviewMode` is set to `"prompt-on-first-use"`, so before I {{action}}, I need to ask:

Where `{{action}}` is the smallest natural description of what triggered the prompt — e.g. _"run the review on PR 15"_, _"open this PR"_, _"continue with the review skills"_. If no specific action is in flight, fall back to _"go any further"_.
...
Closing question (always render _after_ the blockquote, on its own line, plain text — not part of the quote):
